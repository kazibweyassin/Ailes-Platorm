  ...
  useEffect(() => {
    ...
    async function fetchMatches() {
      try {
        ...
        // Check if user came from finder flow
        const finderData = localStorage.getItem('scholarshipFinderData');
        if (finderData) {
          try {
            const data = JSON.parse(finderData);
            // Update user profile with finder data
            ...
            localStorage.removeItem('scholarshipFinderData');
          } catch (err) {
            console.error('Error applying finder data:', err);
          }
        }
                        ...
                        <div className="flex-1">
                          ...
                          <CardDescription className="flex items-center gap-4 text-sm">
                            ...
                            <span className="font-bold text-primary">
                              {match.scholarship.amount ? `${match.scholarship.currency} $${match.scholarship.amount.toLocaleString()}` : `${match.scholarship.currency} Full Funding`}
                            </span>
                            <span>•</span>
